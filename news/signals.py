...
def send_mass_mail(datatuple, fail_silently=False, auth_user=None, auth_password=None, connection=None): # noqa
    """
    ...
    for subject, message, html_message, sender, recipient in datatuple:
        msg = EmailMultiAlternatives(subject, message, sender, recipient, connection=connection) # noqa
        msg.attach_alternative(html_message, "text/html")
        messages.append(msg)
    return connection.send_messages(messages)


@receiver(page_published, sender=Newsletter, dispatch_uid="newsletter.signals.newsletter_notification") # noqa
def newsletter_notification(sender, **kwargs):
    ...
    if page_instance.alias_of_id is None:
        ...
        if page_instance.notify_subscribers is True:
            ...
            html_message = render_to_string('newsletter/email_template.html', {'email_content': page_instance.email_text}) # noqa
            from_email = page_instance.get_parent().subscription_from_email

            emaildata = ()
            for subs in subscribers:
                emaildata = emaildata + ((subject, message, html_message, from_email, [subs.email]),) # noqa
